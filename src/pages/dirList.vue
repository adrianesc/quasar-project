<template>
  <q-page>
    ...
    <div class="row">
      <q-table title="Listado de Directores" @row-click="goTo" :rows="rows" :columns="columns" row-key="name" class="col">
...
export default {
  ...
  methods: {
    goTo (event, row) {
      ...
      // this.$router.push('director?director=' + row.id)
      window.location.href = 'http://localhost:8080/#/director?director=' + row.id
      console.log('aaaaaa ' + row)
    },
    getPosts () {
      this.$axios.get('http://localhost:8069/directores')
        .then((res) => {
          this.rows = res.data
          this.lab = res.data
